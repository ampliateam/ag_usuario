...
export const manejadorDeErrorFirebaseAuthentication = (error: any) => {
...
const manejadorDeError = (error: any) => {
  ...
  if (error.errorInfo.code === 'auth/claims-too-large') {
    ...
    errorDomain.mensajeServidor =
      'El token proporcionado no es válido. Debe ser un token de usuario tipo persona.';
    errorDomain.mensajeCliente =
      'El token proporcionado no es válido. Debe ser un token de usuario tipo persona.';
    ...
    errorDomain.mensajeServidor = 'No se pudo encontrar el usuario.';
    errorDomain.mensajeCliente = 'No se pudo encontrar el usuario.';
  }
