...
export const userRecordToAutenticacionPersona = (userRecord: UserRecord): IAutenticacionPersona => {
  ...
  return {
    ...
    fechaActividad: {
      ...
      fechaUltimoInicioSesion: userRecord.metadata.lastSignInTime
        ? new Date(userRecord.metadata.lastSignInTime)
        : null,
      fechaUltimoMomentoActivo: userRecord.metadata.lastRefreshTime
        ? new Date(userRecord.metadata.lastRefreshTime)
        : null,
    },
    contrasena: {
    ...
    fechaVencimientoToken: userRecord.tokensValidAfterTime
      ? new Date(userRecord.tokensValidAfterTime)
      : null,
